...
export function NameScreen({ userId }: { userId: string }) {
  ...
  const go = async () => {
    ...
    try { setPainter(await ensurePainter(userId, n)); } catch (e: any) { setErr(e?.message ?? 'Failed'); }
    setBusy(false);
  };
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.root}>
      <View style={styles.box}>
        ...
        <Text style={styles.label}>Pick your tag (signs your pieces)</Text>
