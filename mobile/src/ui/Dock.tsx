...
/**
 * Bottom navigation: an arcade cabinet's control strip. A dark plate with a lit top edge, four keys
 * sunk into wells (the current one is a raised purple slab), and the CREATE can key standing proud
...
/** Side keys: a well in the plate; the one you're on is a raised purple slab, pressed keys sink. */
function DockItem({ label, icon, active, onPress }: { label: string; icon: IconName; active: boolean; onPress: () => void }) {
  ...
  const lift = useAnimatedStyle(() => ({ transform: [{ translateY: -4 * s.value + 3 * d.value }] }));
  ...
  return (
    <Pressable onPress={onPress} onPressIn={() => { d.value = withSpring(1, { damping: 20, stiffness: 400 }); }} onPressOut={() => { d.value = withSpring(0, { damping: 14, stiffness: 300 }); }} style={styles.item} hitSlop={2}>
      ...
      <Animated.View style={[styles.itemInner, lift]}>
        ...
        <Text style={[styles.label, { color: active ? C.white : C.faint }]} numberOfLines={1}>{label}</Text>
      </Animated.View>
    </Pressable>
  );
}

/** The centre key: the can silhouette on a green slab that stands above the strip; purple while you're in the camera. */
  ...
  return (
    <Pressable onPress={onPress} onPressIn={() => { d.value = withSpring(1, { damping: 20, stiffness: 400 }); }} onPressOut={() => { d.value = withSpring(0, { damping: 14, stiffness: 300 }); }} hitSlop={4} style={styles.createSlot}>
      <Animated.View style={press}>
        <PixelBox fill={t.fill} hi={t.hi} lo={t.lo} n={6} depth={6} contentStyle={styles.keyIn}>
          <PixelIcon name="create" size={36} color={t.text} alt={active ? C.greenHi : C.greenLo} />
...
const styles = StyleSheet.create({
  ...
  item: { flex: 1, height: KEY_H },
  well: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, backgroundColor: C.ink, borderWidth: 2, borderColor: C.bg2 },
  itemInner: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 3 },
  label: { ...uiLabel(10, 0.4) },
  createSlot: { flex: 1.25, marginTop: -22 },
  keyIn: { height: 74, alignItems: 'center', justifyContent: 'center', gap: 2 },
