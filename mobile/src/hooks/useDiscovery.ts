 ...
 */
export function useDiscovery(pose: React.MutableRefObject<Pose>, autoLock: boolean | ((c: Canvas) => boolean) = true): Discovery {
      ...
      if (loc) {
        for (const c of Object.values(st.canvases)) {
          ...
          if (!known) {
            ...
            const lock = typeof autoLock === 'function' ? autoLock(c) : autoLock;
            if (lock && d <= DISCOVERED_RADIUS_M && facing) {
